#!/bin/bash
#Usage: Py_CF_path.py output_Path Snapnum_min Snapnum_max BoxSize GridSize particle_types[numbers separated by comma]
...
if [ $# != $NUMPARAMS ]; then
	...
	echo ${TARGET}
 	if [ -d ${TARGET} ]; then
	    ...
	    cp ${SCRIPTSPATH}quick_CF.py $TARGET
	    echo Done
	    ...
	    python quick_CF.py ${SNAPNAME%.*} $4 $5 $6
	    echo Done
	    rm ./quick_CF.py
	fi
	    echo Correlation Function Computed
    done
fi
